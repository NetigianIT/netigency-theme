            ...
            <div class="card">
                <div class="card-body">
                    @if (count($portfolio_details) > 0)
                        ...
                        @else
                            <form onsubmit="return btnCheckListGet()" action="{{ route('portfolio-detail.destroy_checked', $id) }}" method="POST">
                                @method('DELETE')
                                @csrf
                                @endif
                        ...
                        <table id="basic-datatable" class="table table-striped dt-responsive nowrap w-100">
                            ...
                            <tbody>
                            @php $desc = count($portfolio_details); $asc=0; @endphp
                            @foreach ($portfolio_details as $portfolio_detail)
                                <tr>
                                    ...
                                    <td>{{ $portfolio_detail->order }}</td>
                                    <td>
                                        <div>
                                            <a href="{{ url('admin/portfolio-detail/'. $id.'/'.$portfolio_detail->id. '/edit') }}" class="mr-2">
                                                <i class="fa fa-edit text-info font-18"></i>
                                            </a>
                                            <a href="#" data-toggle="modal" data-target="#deleteModel{{ $portfolio_detail->id }}">
                                                <i class="fa fa-trash text-danger font-18"></i>
                                            </a>
                                        </div>
                                    </td>
                                ...
                                <!-- Modal -->
                                    <div class="modal fade" id="deleteModel{{ $portfolio_detail->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered modal-sm" role="document">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                ...
                                                @if ($demo_mode == "on")
                                                    <!-- Include Alert Blade -->
                                                        @include('admin.demo_mode.demo-mode')
                                                    @else
                                                        <form class="d-inline-block" action="{{ route('portfolio-detail.destroy', $portfolio_detail->id) }}" method="POST">
                                                            @method('DELETE')
                                                            @csrf
                                                            @endif
            ...
            <div class="modal-content">
                ...
                @if ($demo_mode == "on")
                    <!-- Include Alert Blade -->
                        @include('admin.demo_mode.demo-mode')
                    @else
                        <form action="{{ route('portfolio-detail.store', $id) }}" method="POST">
                            @csrf
                            @endif
