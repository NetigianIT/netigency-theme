...
class FrontendCache
{
    public static function layout(int $languageId): array
    {
        return SiteCache::frontendRemember('layout', $languageId, 'base', SiteCache::TTL_MEDIUM, function () use ($languageId) {
            return [
                ...
                'footer_pages' => Page::footerLinksForLanguage($languageId),
            ];
        });
    }
    ...
    public static function serviceShow(int $languageId, string $slug): array
    {
        ...
        $pageData = SiteCache::frontendRemember('service', $languageId, $slug, SiteCache::TTL_MEDIUM, function () use ($languageId, $slug) {
            ...
            $details = ServiceDetail::query()
                ->select(['id', 'service_id', 'title', 'desc', 'order'])
    ...
    public static function portfolioShow(int $languageId, string $slug): array
    {
        ...
        $pageData = SiteCache::frontendRemember('portfolio', $languageId, $slug, SiteCache::TTL_MEDIUM, function () use ($languageId, $slug) {
            ...
            $recent_posts = static::recentPosts($languageId, 3);
            $portfolio_count_categories = static::portfolioCategoryCounts($languageId);
    ...
    public static function blogShow(int $languageId, string $slug): array
    {
        ...
        $pageData = SiteCache::frontendRemember('blog', $languageId, $slug, SiteCache::TTL_MEDIUM, function () use ($languageId, $slug) {
            ...
            $blog_count_categories = static::blogCategoryCounts($languageId);
    ...
    public static function customPage(int $languageId, string $pageSlug): array
    {
        ...
        $pageData = SiteCache::frontendRemember('page', $languageId, $pageSlug, SiteCache::TTL_MEDIUM, function () use ($languageId, $pageSlug) {
            $page = Page::query()
                ->select([
                    'id',
                    'language_id',
    ...
    {
        return SiteCache::frontendRemember('blog', $languageId, "recent.{$limit}", SiteCache::TTL_MEDIUM, function () use ($languageId, $limit) {
            return Blog::query()
                ->select([
                    'blogs.id',
                    'blogs.title',
    ...
    protected static function blogCategoryCounts(int $languageId)
    {
        return SiteCache::frontendRemember('blog', $languageId, 'category_counts', SiteCache::TTL_MEDIUM, function () use ($languageId) {
            return Blog::query()
    ...
    protected static function portfolioCategoryCounts(int $languageId)
    {
        return SiteCache::frontendRemember('portfolio', $languageId, 'category_counts', SiteCache::TTL_MEDIUM, function () use ($languageId) {
    ...
    /**
     * Render a detail view once and cache the HTML string.
     */
    public static function rememberShowHtml(string $group, int $languageId, string $suffix, string $view, callable $dataResolver): string
    {
        return SiteCache::frontendRememberHtml($group, $languageId, $suffix, SiteCache::TTL_MEDIUM, function () use ($view, $dataResolver) {
            return view($view, $dataResolver())->render();
        });
    }
}
